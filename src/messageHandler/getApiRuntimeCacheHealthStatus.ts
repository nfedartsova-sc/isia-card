...
const REQUIRED_FIELDS = Object.keys({
  isiaCode: null,
  name: null,
  title: null,
  countryCode: null,
  ...
  webSite: null,
  expirationDate: null,
} satisfies Record<keyof ISIACardResponseData, null>) as Array<keyof ISIACardResponseData>;
...
const checkApiCacheStatus = async (): Promise<{
  ...
  try {
    ...
    // Try to find the cached response for API_ENDPOINT
 ...
 */
const getApiRuntimeCacheHealthStatus = async (
  eventSource: MessagePort | Client | ServiceWorker | null
) => {
